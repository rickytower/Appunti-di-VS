...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\bitem{Funzione parzialmente corretta}una funzione è \textit{parzialmente corretta} se per ogni ingresso che soddisfa la pre-condizione, \ul{se la funzione termina}, allora l'uscita soddisfa la post-condizione\footnote{Nota: stiamo parlando degli ingressi e uscite delle funzioni di un programma, non dell'ingresso e dell'uscita di un programma.}. Si dice \textit{``parzialmente''} perché si ammette la terminazione della funzione. Il termine \textit{``corretta''} viene usato perché pre e post-condizione sono valide. 
		
		\bitem{Funzione totalmente corretta}una funzione è \textit{totalmente corretta} se per ogni ingresso che soddisfa la pre-condizione, \ul{la funzione termina} e l'uscita soddisfa la post-condizione.
	\end{itemize}
...
\begin{enumerate}
	...
	\item delle formule $\{\phi_1\dots\phi_k\}$ dette \textbf{condizioni di verifica} che sono formule la cui validità assicura che il programma è \textit{parzialmente corretto }se si usa \ul{solo} il \textit{metodo delle asserzioni induttive} e \textit{totalmente corretto }se si usa \ul{anche} il \textit{metodo delle funzioni d'ordine}. Il metodo delle asserzioni induttive e il metodo delle funzioni d'ordine sono \textit{pienamente automatizzabili}, mentre la specifica no.
\end{enumerate}
